...
export class HelpsComponent implements OnInit, OnDestroy {
  ...
  // get all the helps
  getAllHelps() {
    this.helpSub = this.helpService.getAllHelps().subscribe(
      (helps: Help[]) => {
        this.helps = helps;
      },
  ...
  createHelp(help: Help) {
    this.helpSub = this.helpService.createHelp(help).subscribe(
      (newHelp) => {
  ...
  deleteHelp(help: Help) {
    ...
    this.helpSub = this.helpService.deleteHelp(help).subscribe(
      () => {
        this.snackBar.open(
          'Help deleted!',
          'Close',
  ...
  editHelp(help: Help) {
    ...
    this.helpSub = this.helpService.editHelp(id!, help).subscribe(
      (newHelp) => {
        this.helps.unshift(newHelp);
        console.log('Edited');
        this.showForm = false;
